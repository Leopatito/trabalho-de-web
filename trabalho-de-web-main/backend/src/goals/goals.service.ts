...
export class GoalsService extends BaseService<Goal> {
  ...
  public async updateGoal(
    ...
    if (updateGoalDto.accumulatedValue && updateGoalDto.accountId) {
      ...
      const isInvestment = goal.type === GoalType.INVESTMENT;
      const tx = this.standardTransactionsService.getRepository().create({
        account,
        amount: updateGoalDto.accumulatedValue,
        date: new Date(),
        description: `Movimentação da meta: ${goal.type}`,
        type: 'Standard', 
        metaType: isInvestment ? 'investment' : 'withdraw',
      });

      await this.standardTransactionsService.getRepository().save(tx);
